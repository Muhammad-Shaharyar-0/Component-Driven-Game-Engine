...
void GodModeComponent::Start()
{
	...
	_owner->RegisterListener("godmode", this);

	RenderComponent* rc = (RenderComponent*)_owner->GetComponent("render");
	if (rc)
	{
		_objectColour = rc->GetColour();
	}

	CollisionComponent* cc = (CollisionComponent*)_owner->GetComponent("collision");
	if (cc)
	{
...
{
	_owner->UnregisterListener("godmode", this);
}
